...
在 `04_群美相館\`（跟 `website\` 同一層）已經依照首頁的分類編號先建好對應的原始照片資料夾，數字編號跟網站上的分類順序完全對應，之後拍照或有廠商圖片時直接丟進對應資料夾就好，不用先做任何處理：

```
04_群美相館\
├── 01_姓名貼紙\
│   ├── 姓名貼圖\                  # 274 張商品照片
│   └── 姓名貼紙目錄_含分類與圖片欄位.xlsx
├── 02_相片沖印\
│   ├── 洗相片\
│   └── 拍立得底片\
├── 03_證件照大頭照\
│   ├── 護照\ 簽證\ 駕照\ 學生證\ 國家考試檢定\ 身分證\ 選舉半身照\
├── 04_印章類\
│   ├── 木頭印章\ 連續印章\ 肚臍章\
├── 05_加工服務\
│   ├── 護貝\ 裝框\
...
丟進去之後跟我說一聲，我會負責壓縮、搬進網站、寫資料、上架，不需要自己動手改網站檔案。

**技術備註**：`scripts/` 底下處理姓名貼紙資料的腳本（`xlsx-to-json.ps1`、`add-gift-bag-flag.ps1`、`resize-images.ps1`）是用資料夾名稱開頭的「01」數字去找 `01_姓名貼紙\` 這個資料夾，不是用中文字比對（Windows PowerShell 5.1 在沒有 BOM 的 `.ps1` 檔案裡讀中文字容易亂碼，所以刻意避開）。如果之後要再幫其他分類寫類似的自動化腳本，一樣建議用資料夾編號而不是中文名稱來定位。
...
然後開瀏覽器連到 `http://localhost:8793`（port 號寫在 `scripts/serve.ps1` 開頭）。
